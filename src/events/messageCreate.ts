...
export const event: Event<TypeEvents.Bot> = {
	...
	code: async (message: djs.Message) => {
		...
		const _prefixes = [db?.prefix || config.prefix, `<@${message.client.user!.id}`, `<@!${message.client.user!.id}`]
		const prefix = _prefixes.find((p) => message.content.toLowerCase().startsWith(p))
		if (!prefix) return
		const args = message.content.slice(prefix.length).trim().split(/ +/)
		const probably = args.shift()?.toLowerCase()
		if (!probably) return
		const command = commands.find((cmd) => cmd.data.names.includes(probably!))
		if (!command) return
		...
		if (command.data.isDev && !config.devs.includes(d.message.author.id)) return
		...
		if (!util.getCooldowns().has(command.data.names[0]))
			util.getCooldowns().set(command.data.names[0], new djs.Collection<any, any>())
		...
		if (timestamps.has(message.author.id)) {
			if (Date.now() < timestamps.get(message.author.id) + amount) {
				const timeLeft = (timestamps.get(message.author.id) + amount - Date.now()) / 1000
				return util.defer(d, {
					content:
						`<@${message.author.id}>, ` +
						d.util
		...
		if (command.data.permissions.bot.length) {
			const botperms = command.data.permissions.bot.map((p) => '`' + p + '`').join(', ')
			if (!message.guild!.members.me!.permissions.has(command.data.permissions.bot))
				return d.util.throwError(
					d,
		...
		if (command.data.permissions.user.length) {
			const botperms = command.data.permissions.user.map((p) => '`' + p + '`').join(', ')
			if (!message.guild!.members.me!.permissions.has(command.data.permissions.bot))
				return d.util.throwError(
					d,
