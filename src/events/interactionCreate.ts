...
import { Data, Event, SlashInteraction, TokyoInteraction, TypeEvents, Types } from '../../index'
...
export const event: Event<TypeEvents.Bot> = {
	...
	code: async (interaction: TokyoInteraction | djs.CommandInteraction) => {
		...
		if (interaction.type === djs.InteractionType.ApplicationCommand) {
			...
			const int = ints.find((i) => i.data.name === interaction.commandName)
			if (!int) return
		...
		} else if (interaction.type !== djs.InteractionType.ApplicationCommandAutocomplete) {
