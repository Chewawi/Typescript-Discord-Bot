...
export const command: Command<Types.Slash> = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName('embed').setDescription('Send an embed message through the bot.').setRequired(false)
		)
		.addStringOption((option) =>
			option.setName('message').setDescription('Send a normal message through the bot.').setRequired(false)
		),
	code: async (d) => {
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
			new ButtonBuilder()
				...
				.setLabel(d.util.look(d.idiom, 'Enviado por:', 'Sent by:', 'Enviado por:') + ` ${d.int.user!.tag}`)
				.setDisabled(true)
		)

		//@ts-ignore
		if (!d.int.member!.permissions!.has(PermissionFlagsBits.ManageMessages)) {
			d.util.throwError(d, '8', '9', '0')
		} else {
