...
export const command: Command<Types.Prefix> = {
	...
	code: async (d) => {
		...
		try {
			...
			d.message.channel
				.send({
					content: `\`\`\`ts\n${lines.map((x, i) => `${i.toString().padStart(3, '0')} | ${x}`).join('\n')}\`\`\``
						.replaceAll(d.client.token!, '[UNI_TOKEN]')
						.replaceAll(process.env.NAME!, '[UNI]'),
					components: [row]
				})
				.then((m) => {
			...
			const lins = err.toString().split(/\n/g)
			d.message.channel.send(
				`\`\`\`ansi\nError:\n${lins
					.map((x, i) => `${i.toString().padStart(3, '0')} | ${x}`)
					.join('\n')
					.replaceAll(d.client.token!, '[UNI_TOKEN]')
					.replaceAll(process.env.NAME!, 'OMG')}\`\`\``
			)
		}
	}
}
