	...
	code: async (d) => {
		if (d.args[0]) {
			...
			const embed = new EmbedBuilder()
				...
				.setFields(
					{
						name: d.util.look(d.idiom, 'Descripción:', 'Description:', 'Descrição:'),
						value: cmd.data.explan[d.idiom],
						inline: true
					},
					{
						name: d.util.look(d.idiom, 'Categoría:', 'Category:', 'Categoria:'),
					...
					},
					{
						...
						value:
							'```fix\n' +
							d.prefix +
							`${cmd.data.names.length > 1 ? '[' + cmd.data.names.join('|') + ']' : cmd.data.names[0]} ${
								cmd.data.usage?.[d.idiom] ? cmd.data.usage[d.idiom] : ''
							}` +
							'```'
					}
				)
			...
			const embed = new EmbedBuilder()
				...
				.setFields(
					{
						name: '<:info:1000241343584538765> ' + d.util.look(d.idiom, 'Información:', 'Info:', 'Informação:'),
						value: `_ _ ${d.util.look(d.idiom, 'Versión:', 'Version:', 'Versão:')} \`v0.2.1\``
					},
					{
						name:
							'<:icons_link:1014155773036728382> ' +
							d.util.look(d.idiom, 'Links útiles:', 'Useful links:', 'Links úteis:'),
						value: `_ _ <:dot:999761724498526319> [\`${d.util.look(d.idiom, 'Invitación', 'Invite', 'Convite')}\`](${
							d.config.invite
						})\n_ _ <:dot:999761724498526319> [\`${d.util.look(d.idiom, 'Soporte', 'Support', 'Suporte')}\`](${
							d.config.discord
						})`
					},
					{ name: '\u200b', value: '\u200b' },
			...
			const row = new ActionRowBuilder<SelectMenuBuilder>().addComponents(
				new SelectMenuBuilder()
					...
					.setPlaceholder(
						d.util.look(d.idiom, 'Selecciona una categoría.', 'Select a category.', 'Selecione uma categoria.')
					)
					.setOptions(
						{
							label: 'Home',
			...
			d.message.channel.send({ embeds: [embed], components: [row] }).then((message) => {
				...
				collector.on('collect', (i) => {
					...
					if (i.values[0] === 'home') {
						embed
							...
							.setTitle(null)
						//@ts-ignore
						i.update({ embeds: [embed] }).catch((e) => d.util.solve(d))
					} else {
						const cmds = d.util
							.getCollection<Types.Prefix>('prefix')
						...
						embed
							.setTitle(`${i.values[0]}`)
							.setFields({
								name: d.util.look(d.idiom, 'Comandos', 'Commands', 'Comandos'),
								value: `${cmds.length ? cmds.map((c) => '**`' + c + '`**').join(', ') : 'Not added yet.'}`
							})
							.setFooter({
								text: `${cmds.length} ${d.util.look(d.idiom, 'comandos.', 'commands.', 'comandos.')}`,
								iconURL: d.client.user!.displayAvatarURL()
							})
						//@ts-ignore
						i.update({ embeds: [embed] }).catch((e) => d.util.solve)
					}
				})
				collector.on('end', () => {
