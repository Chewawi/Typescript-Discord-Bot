...
export const command: Command<Types.Prefix> = {
	data: new PrefixBuilder().setNames('8ball', 'eigthball').setCategory('Fun').setArgs(1),
	//@ts-ignore
	code: async (d) => {
		const args = d.args.join(' ')
		const encoded = encodeURIComponent(args)
		const response = await axios
			.get<Record<string, any>>(`https://api.miduwu.ga/json/8ball?text=${encoded}&idiom=${d.idiom}`)
			.catch((e) => null)
		...
		const embed = new EmbedBuilder()
			...
			.addFields(
				//@ts-ignore
				{
					name: d.util.look(d.idiom, 'Pregunta', 'Question', 'Pergunta'),
					value: `_ _ ${args}`
				},
				//@ts-ignore
				{
					name: d.util.look(d.idiom, 'Respuesta', 'Answer', 'Resposta'),
