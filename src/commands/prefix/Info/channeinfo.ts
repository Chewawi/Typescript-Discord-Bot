...
import { Errors, Emojis } from 'aux/constants'
import { EmbedBuilder, GuildTextBasedChannel } from 'discord.js'

export const command: Command<Types.Prefix> = {
	data: new PrefixBuilder().setNames('channel', 'channelinfo', 'channel-info'),
	code: async (d) => {
		const channel = d.args[0]
			? ((await d.util.getChannel(d, d.args[0])) as any)
			: (d.message.channel as GuildTextBasedChannel)
		if (!channel) return d.util.throwError(d, ...Errors.channel_found)
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				`_ _ **${d.util.look(d.idiom, 'Nombre', 'Name', 'Nome')}:** \`${channel!.name}\`\n_ _ **ID:** \`${
					...
					d.message.guild!.channels.cache.get(channel!.parentId!)?.name
			...
			.setTimestamp()
			.setFooter({
				text: d.message.guild!.name,
				iconURL: d.message.guild!.iconURL({ forceStatic: false }) || null
			})

		channel?.lastMessageId &&
			embed.addFields({
				name: `_ _ ${d.util.look(
					d.idiom,
					'Último mensaje del canal:',
					'Last message of the channel:',
					'Última mensagem do canal:'
				)}`,
				value: `\`${last_msg.author!.tag}: ${last_msg.content.slice(0, 400)}\``
			})

		d.message.reply({ embeds: [embed] }).catch((e) => d.util.solve(d))
	}
}
