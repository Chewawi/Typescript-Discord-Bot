...
export const command: Command<Types.Prefix> = {
	...
	code: async (d) => {
		...
		const res = await axios
			.get<Record<string, any>>(`https://api.miduwu.ga/json/translate?source=en&target=${encoded1}&text=${encoded2}`)
			.catch((e) => null)
		...
		const embed = new EmbedBuilder()
			...
			.setTitle(d.util.look(d.idiom, 'Traducción', 'Translate', 'Tradução'))
