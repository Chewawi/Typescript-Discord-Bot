...
var PaginationEmbed = class extends discord_js.EmbedBuilder {
  constructor(options = {}) {
    super();
  ...
  addEmbeds(embeds, template) {
    if (template) {
      embeds = embeds.map((embed) => template(discord_js.EmbedBuilder.from(embed)));
    }
...
var Pagination = class extends PaginationEmbed {
  ...
  async render() {
    if (this.interaction instanceof discord_js.BaseInteraction && (this.interaction.replied || this.interaction.deferred)) {
  ...
  async followUp() {
    const payload = this.ready();
    if (!(this.interaction instanceof discord_js.BaseInteraction))
